...
ipfs.on('ready', async () => {
  ...
  exchange = new TradingPairExchange('test-db', ipfs, 1);
  await exchange.init();
  // Test that can retrieve no orders (i.e. order book empty)
  num_tests_run++;
  try {
    ...
    num_tests_failed++;
    console.log("FAILED: getAllOrders: Test 1");
    console.log(err.name, ": ", err.actual, err.operator, err.expected);
  }
  exchange.db.close();
  ...
  exchange = new TradingPairExchange('test-db', ipfs, 1);
  await exchange.init();
  let ts_82 = await exchange.addOrder(new Order(true, 10, 100, "#82", undefined));
  // Test that can retrieve single order
  num_tests_run++;
  try {
    let orders = exchange.getAllOrders();
    assert.strictEqual(orders.length, 1);
    assert.strictEqual(
          JSON.stringify(new Order(true, 10, 100, "#82", ts_82)),
          JSON.stringify(orders[0]));
    num_tests_passed++;
  } catch (err) {
    num_tests_failed++;
    console.log("FAILED: getAllOrders: Test 2");
    console.log(err.name, ": ", err.actual, err.operator, err.expected);
  }
  exchange.db.close();
  ...
  exchange = new TradingPairExchange('test-db', ipfs, 1);
  await exchange.init();
  let ts_77 = await exchange.addOrder(new Order(true, 10, 100, "#77", undefined));
  let ts_78 = await exchange.addOrder(new Order(true, 15, 100, "#78", undefined));
  let ts_79 = await exchange.addOrder(new Order(true, 5, 110, "#79", undefined));
  let ts_80 = await exchange.addOrder(new Order(false, 8, 120, "#80", undefined));
  let ts_81 = await exchange.addOrder(new Order(false, 12, 130, "#81", undefined));
  // Test that can retrieve multiple orders
  num_tests_run++;
  try {
    let orders = exchange.getAllOrders();
    assert.strictEqual(orders.length, 5);
    assert.strictEqual(
      JSON.stringify(new Order(true, 10, 100, "#77", ts_77)),
      JSON.stringify(orders[0]));
    assert.strictEqual(
      JSON.stringify(new Order(true, 15, 100, "#78", ts_78)),
      JSON.stringify(orders[1]));
    assert.strictEqual(
      JSON.stringify(new Order(true, 5, 110, "#79", ts_79)),
      JSON.stringify(orders[2]));
    assert.strictEqual(
      JSON.stringify(new Order(false, 8, 120, "#80", ts_80)),
      JSON.stringify(orders[3]));
    assert.strictEqual(
      JSON.stringify(new Order(false, 12, 130, "#81", ts_81)),
      JSON.stringify(orders[4]));
    num_tests_passed++;
  } catch (err) {
    num_tests_failed++;
    console.log("FAILED: getAllOrders: Test 3");
    console.log(err.name, ": ", err.actual, err.operator, err.expected);
  }
  exchange.db.close();

  exchange = new TradingPairExchange('test-db', ipfs, 0.01, 2, 1);
  await exchange.init();
  ...
  if (num_tests_passed === num_tests_run) {
    ...
    console.log("NOT ALL getAllOrders TESTS PASSED!");
    console.log(num_tests_passed + " out of " + num_tests_run + " tests passed.");
  }
